...
const published = db.prepare(`
  ...
  WHERE il.status = 'published'`).all();

const findings = [];
const flag = (link, axis, detail) => findings.push({ axis, link_key: link.link_key, eik: link.eik, detail });
...
for (const l of published) {
  ...
  else if (rec.valid.size !== 1) flag(l, 'A_multi_eik', `key ${l.entity_key} → ${rec.valid.size} valid ЕИК {${[...rec.valid].join(',')}}; names {${[...rec.names].join(' | ')}}`);
  else if (![...rec.valid][0] || [...rec.valid][0] !== l.eik) flag(l, 'A_eik_mismatch', `key resolves to ${[...rec.valid][0]} but link published ${l.eik}`);

  // B. Row integrity: the stored bidder must itself be the valid, published ЕИК.
  if (l.bidder_eik !== l.eik) flag(l, 'B_bidder_eik', `bidder_id eik ${l.bidder_eik} != link eik ${l.eik}`);
  if (!l.bidder_eik_valid) flag(l, 'B_eik_invalid', `published on eik_valid=0 bidder ${l.bidder_name}`);
  ...
  if (l.publish_tier === 'B_distinctive' && nameDistinctiveness(l.entity_key) !== 'distinctive')
    flag(l, 'C_not_distinctive', `tier B_distinctive but nameDistinctiveness=${nameDistinctiveness(l.entity_key)} (${l.bidder_name})`);
}
...
for (const l of nonExact) {
  ...
  const hit = rows.find((r) => {
    ...
    const nameHit = companyCandidates(t).some((c) => companyNameKey(c) === winnerKey) || companyNameKey(t).includes(winnerKey);
    return (l.match_method === 'declared_eik' && eikHit && nameHit) || (l.match_method === 'extracted_name' && nameHit);
  });
  provenance.push({ link_key: l.link_key, eik: l.eik, method: l.match_method, winner: l.bidder_name, evidence: hit ? hit.entity_raw : null });
}
...
for (const p of provenance) {
  ...
  console.log(`      evidence: ${p.evidence ? JSON.stringify(p.evidence) : 'NONE FOUND (cross-check would now fail!)'}`);
}
