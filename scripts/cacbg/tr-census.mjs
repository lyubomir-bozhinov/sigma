...
const LEGAL_FORM = {
  EOOD: 'ЕООД', OOD: 'ООД', EAD: 'ЕАД', AD: 'АД', ET: 'ЕТ', KD: 'КД', K: 'КД', SD: 'СД',
  KCHT: 'КДА', IAD: 'ИАД', IEAD: 'ИЕАД', DPK: 'ПК', EDPK: 'ЕПК', CC: 'кооперация', ASSOC: 'СНЦ', FOUND: 'фондация',
};
...
const bgName = (companyName, legalForm) => `${String(companyName ?? '').trim()} ${LEGAL_FORM[legalForm] ?? String(legalForm ?? '').trim()}`.trim();
...
export function extractEntity(rec) {
  ...
  const name = rec.CompanyName != null ? bgName(rec.CompanyName, rec.LegalForm) : (rec.name ?? null);
...
function* records(dump) {
  ...
  if (trimmed.startsWith('{') || trimmed.startsWith('[')) {
    ...
    if (Array.isArray(deeds)) { for (const d of deeds) yield d?.['$'] ?? d; return; }
...
export function promote(db, census, { dryRun = false } = {}) {
  ...
  for (const l of held) {
    ...
    if (eiks && eiks.size === 1 && eiks.has(l.eik)) { would.push(l.link_key); if (!dryRun) upd.run(l.link_key); }
  }
...
function dumpFiles() {
  ...
  if (dir) return fs.readdirSync(dir).filter((f) => /\.(json|jsonl)$/.test(f)).map((f) => path.join(dir, f));
  return DUMP ? DUMP.split(',').map((s) => s.trim()).filter(Boolean) : [];
}
...
if (isMain) {
  ...
  if (!files.length) {
    console.log('no --dump/--dump-dir provided; census not run. Provide TR open-data files to promote tier-C links.');
  } else {
    const db = new DatabaseSync(DB);
    const census = buildCensus(files);
    const eikTotal = [...census.values()].reduce((n, s) => n + s.size, 0);
    console.log(`census: ${files.length} file(s) → ${census.size} distinct name-keys / ${eikTotal} ЕИК`);
    const { promoted, stillHeld, would } = promote(db, census, { dryRun });
    console.log(`${dryRun ? 'DRY-RUN would promote' : 'tier-C promotions'}: ${promoted} published, ${stillHeld} still held (non-unique or namesake mismatch)`);
    if (dryRun && would.length) console.log('  would-promote link_keys:\n   ' + would.join('\n   '));
    db.close();
  }
}
