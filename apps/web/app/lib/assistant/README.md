...
wrangler secret put BGGPT_API_KEY --name <worker>                            # per worker script; никога не се комитва
# `AI` (Workers AI) не изисква създаване на ресурс — account capability; включи Workers AI за акаунта.
# Vectorize/R2/AI-bindings сочат към ресурси по ИМЕ, споделени за акаунта; secret-ите са per-worker-script.
```

**Среди dev + preview (този repo).** Vectorize индексите и R2 кофите са глобални за акаунта, затова
**preview не пресъздава backend ресурси** — ефемерните preview worker-и (`sigma-pr-<N>`) се връзват към
*същите* `sigma-assistant` / `sigma-reports` / D1 на `dev` по име. Provisioning-ът горе се прави **веднъж за
dev**. Внимание: **secret-ите са per-worker-script** — `sigma-pr-<N>` НЕ наследяват `BGGPT_API_KEY` от
`sigma-dev`, така че preview pipeline-ът трябва сам да го `wrangler secret put` от repo/CI secret (не на
ръка всеки път). Render-ът пази това: `SIGMA_CSV_CACHE_NAME` преименува само `CSV_CACHE`, а `sigma-reports`
остава споделен (`SIGMA_REPORTS_NAME` нарочно неподаден) — виж [scripts/wrangler-render.mjs](../../../../../scripts/wrangler-render.mjs).

**Seed на схема-корпуса (`indexSchemaCorpus`).** Новосъздаденият индекс е празен; докато не се напълни,
RAG grounding-ът връща 0 chunk-а и системният prompt пада обратно към статичния `describeSchema()`. Seed-ва
се през едноразовия route `POST /assistant/reindex`, който вика `indexSchemaCorpus(env.AI, env.VECTORIZE)`
с реалните bindings (затова embedding-ите са идентични на тези, които `retrieveSchemaContext` чете). Route-ът
е **изключен по подразбиране** и се отваря само при наличен `ASSISTANT_SEED_TOKEN` (иначе 404 — без сигнал за
съществуване). Идемпотентен е (upsert по стабилен id):

```bash
wrangler secret put ASSISTANT_SEED_TOKEN --name <worker>     # high-entropy; напр. `openssl rand -hex 32`
curl -fsS -X POST https://<worker-url>/assistant/reindex \
  -H "Authorization: Bearer $ASSISTANT_SEED_TOKEN"           # → {"ok":true,"indexed":<N>}
```
