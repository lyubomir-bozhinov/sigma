...
| `agent.ts`                  | Vercel AI SDK glue: чат моделът (OpenRouter) през AI Gateway + `streamText` | §2/§9.5 | typecheck |
...
средата с асистента осигурете: `ASSISTANT_API_KEY` (secret, `wrangler secret put`), Cloudflare AI
Gateway + var-овете `AI_GATEWAY_BASE_URL`/`AI_GATEWAY_ID` (routing-ът през Gateway е **задължителен** —
без URL асистентът връща 503, не вика доставчика директно), Vectorize индекс `sigma-assistant`, R2 кофа
`sigma-reports`, и еднократно индексиране на схема-корпуса (`indexSchemaCorpus`).
...
wrangler secret put ASSISTANT_API_KEY                                            # интерактивно; никога не се комитва
# AI Gateway: създай gateway-а и насочи var-овете към него (OpenAI-съвместимия OpenRouter upstream):
#   AI_GATEWAY_BASE_URL=https://gateway.ai.cloudflare.com/v1/<account>/<gateway>/openrouter/v1
#   AI_GATEWAY_ID=<gateway>   # slug-ът; през него минават и Workers AI embeddings-ите
