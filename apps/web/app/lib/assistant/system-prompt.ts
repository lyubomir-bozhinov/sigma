...
export const HEADLINE_TOTALS_RULE =
  'ВОДЕЩО ЧИСЛО В КАРТАТА: Когато справката е списък или разбивка (`table`, `bar`, `flows`, ' +
  '`timeseries`) и ИМА смислено обобщаващо число за въпроса, започни с водещ `totals` блок — ' +
  'обикновено ОБЩА СУМА по `amount_eur` за ЦЕЛИЯ въпрос (не само за показаните/отрязани редове) ' +
  'и/или БРОЙ на договорите (не брой редове). Това число се показва в компактната карта на чата. ' +
  'Изчисли го с ОТДЕЛНА агрегираща заявка COUNT/SUM и реферирай нейния хендъл — НИКОГА не сочи ' +
  '`totals` към ред от списъка и не пиши числото в прозата. Пропусни водещия `totals`, когато ' +
  'няма едно обобщаващо число (напр. `flows`, или `timeseries`, където редовете са периоди). ' +
  'Това водещо число не изисква `reconcile_rollup`, освен ако е единичен грейн, който rollup покрива.';
