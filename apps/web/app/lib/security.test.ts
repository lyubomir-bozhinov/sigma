...
describe('securityHeaders CSP', () => {
  it('emits a strict nonce script-src and the documented style-src', () => {
    ...
    expect(csp).toContain(
      "script-src 'self' https://challenges.cloudflare.com https://*.challenges.cloudflare.com 'nonce-test-nonce'",
    );
    // Turnstile (H3 bot gate) needs its origin AND challenge subdomains (brunhild.* etc., Firefox PAT flow).
    expect(csp).toContain(
  ...
  it('uses script hashes (not a nonce) for the edge-cached variant', () => {
    ...
    expect(csp).toContain(
      "script-src 'self' https://challenges.cloudflare.com https://*.challenges.cloudflare.com 'sha256-abc123'",
    );
