...
**Какво стига до клиента — филтриран stream.** Сървърът НЕ пуска суровия SDK stream. Вътрешният tool
loop (`run_sql` — SQL заявката, сглобявана token по token, и суровите редове от D1 — `reconcile`,
schema lookups, reasoning) се филтрира server-side (`lib/assistant/stream-phase.ts`, strict allowlist).
До dock-а достигат само: разговорният `text`, структурните маркери, `emit_report` (продуктът) и по
една **фаза** на ход. Грешките при валидация на неуспешна справка (`emit_report` `{ ok: false }`) се
редактират до `errors: []` — детайлите остават server-side.

Фазата е `transient` data-part: доставя се през `useChat().onData`, но НЕ влиза в `message.parts`,
затова никога не се запазва в транскрипта. Затворен енум ключ — без имена на инструменти, SQL или
свободен текст:

```ts
{ type: 'data-phase', data: { phase: 'thinking' | 'querying' | 'composing' }, transient: true }
```

Dock-ът мапва ключа към български етикет (Обмислям… / Търся в данните… / Съставям справка…) и го
показва като ефимерен ред в aria-live лога; изчиства се при сетълване на хода.
...
Fixture (примерна последователност от raw SSE chunk-ове **преди** филтъра, за reference):
[`fixtures/sse-stream.fixture.txt`](../../apps/web/app/lib/assistant/fixtures/sse-stream.fixture.txt).
Показва суровия SDK stream (вкл. `run_sql` частите) — това, което филтърът от `stream-phase.ts` НЕ
пуска до клиента; за филтрирания изход виж горното.
...
| v2     | 2026-07-02 | Chat stream-ът се филтрира до фази + текст + справка; добавен `data-phase` transient part. |
