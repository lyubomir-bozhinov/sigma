# ADR-0016: Public conflict surface shows ONLY declared private ownership

- Status: Accepted
- Date: 2026-07-06
- Deciders: lb, Claude
- Supersedes (display only): [ADR-0013](0013-private-interest-vs-ex-officio-classification.md)
- Related: [ADR-0001](0001-scope-and-certainty-bar.md), [ADR-0014](0014-conflict-explorer-surface-posture.md); `packages/db/src/queries/related-persons.ts`, `apps/web/app/routes/conflict*.tsx`

## Context

ADR-0013 classified each link as `private_ownership` / `ex_officio_board` / `management_role` and the first UI
...
is not to surface it at all. The certainty bar (ADR-0001) is about *what is true*; this is about *what is fair to
...
2. The `interest_class` classification (ADR-0013) is **retained in the data**, but its job flips from *"drive a
...
- The raw €2.3 B / private €330.5 M distinction (ADR-0013) is moot for the UI: only the €330.5 M private set is
...
- ADR-0013's *display* decision (separate ex-officio list) is superseded; its *classification* decision stands.
  ADR-0014/0015 still hold (interest_links-only read model, noindex-until-go-live, temporal dating, divestment
