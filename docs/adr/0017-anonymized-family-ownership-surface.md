# ADR-0017: Anonymized close-relative (family) ownership surface

- Status: Accepted
- Date: 2026-07-07
- Deciders: lb, Claude
- Related: [ADR-0001](0001-scope-and-certainty-bar.md), [ADR-0004](0004-pii-posture.md), [ADR-0013](0013-private-interest-vs-ex-officio-classification.md), [ADR-0016](0016-public-surface-private-ownership-only.md); `scripts/cacbg/parse.mjs`, `scripts/cacbg/load.mjs`, `packages/db/src/queries/related-persons.ts`, `apps/web/app/routes/conflict*.tsx`
...
ADR-0004 kept third-party (related-person) data **internal-only** — it is PII. But the *link* here does not
...
- **Materiality gate (deterministic):** only closely-held forms (ООД/ЕООД/ЕТ/КД/СД…) reach the surface; listed
  ...
  shares → €88M" defamation trap; ADR-0016). Table-type + legal-form, not a post-hoc heuristic.
...
- Consistent with ADR-0001 (certainty 1.0) and ADR-0016 (fair-to-publish ≠ merely-true): a family link is only
