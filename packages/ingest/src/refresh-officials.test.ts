...
INSERT INTO interest_links
  ...
  ('il:s','person:ИВАН|111','person:ИВАН','eik:111','111','АЛФА ООД','exact_name_key','v1','B_distinctive','owns','private_ownership',1,'exact',1,'2020','2023',1,90000,'2021','2021','published'),
  ('il:f','person:ИВАН|111|family','person:ИВАН','eik:111','111','АЛФА ООД','exact_name_key','v1','B_distinctive','related','family_ownership',0,'none',1,'2020','2023',1,90000,'2021','2021','published'),
...
describe('ETL refresh-slice officials batch (the live parse-then-execute path)', () => {
  it('parser keeps the officials INSERT one statement, and the batch populates search_index from interest_links', () => {
    ...
    try {
      ...
      // Иван is indexed once, €90k (self+family collapsed → NOT doubled); Гео (held only) is absent.
      expect(officials).toHaveLength(1);
      expect(officials[0]!.ref).toBe('person:ИВАН');
      expect(officials[0]!.title).toBe('Иван Тестов');
      expect(officials[0]!.amount).toBe(90000);
    } finally {
